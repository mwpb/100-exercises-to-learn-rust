...
pub async fn pong(mut receiver: mpsc::Receiver<Message>) {
    loop {
        if let Some(msg) = receiver.recv().await {
            ...
            let (sender, new_receiver) = mpsc::channel(120);
            msg.response_channel
                .send(Message {
                    payload: "pong".into(),
                    response_channel: sender,
                })
                .await // now we yield if we don't receive a message
                .unwrap();
...
mod tests {
    ...
    #[tokio::test]
    async fn ping() {
        let (sender, receiver): (Sender<Message>, Receiver<Message>) = mpsc::channel(120);
        let (response_sender, mut response_receiver) = mpsc::channel(120);
        sender
            .send(Message {
                payload: "pong".into(),
